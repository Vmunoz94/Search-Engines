...
OUTPUT_FILE_NAME = "positionalInvertedIndex.txt"
...
def write_to_file(file):
    with open(file, "w") as file:
        file.write("[Term: Document Frequency] -> [DocID, Term Frequency: Position1, Position2...], [...]\n")
        for term in sorted(master_table.keys()):
            file.write(f"[{term}: {len(master_table[term])}] -> ")
            for posting in master_table[term]:
                positions = get_term_position(term, posting)
                file.write(f"[{posting}, {len(positions)}: {positions}], ")
            file.write("\n")
    return

# Brute Force, search term through every posting and count its position
def get_term_position(term, posting):
    doc = ""
    position = []
    index = 0
    docID = 0
    ...
    with open(INPUT_FILENAME, "r") as file:
        for line in file:
            ...
            if match_doc_start:
                docID = int(match_doc_start.group(1))
                continue
            elif docID == posting:  # read file and look for the matching posting list
                doc += line         # save entire document in doc
                if match_doc_end:
                    break

        doc = tokenize(doc)
        doc = porter_stemmer(doc)
        # search through doc word by word and save the positions of the term 
        for word in doc:
            index += 1
            if word == term:
